...
#define METADATA (LARGE_CLUSTERS + SMALL_CLUSTERS) * 32
...
void* mzalloc_small(size_t size)
{
    ...
    while (1)
    {
        ...
        {
            int res = bitmapRecodeSuccessive(cur->small + i, size / SMALL_SHARD_SIZE);
...
void* mzalloc_large(size_t size)
{
    ...
    while (1)
    {
        ...
        {
            int res = bitmapRecodeSuccessive(cur->large + i, size / LARGE_SHARD_SIZE);
            shard = res == -1 ? shard : res;
        }
